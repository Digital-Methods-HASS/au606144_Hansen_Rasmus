...
for (i in PopulationMun$Municipalities){
  x <- AbortNumDF %>% select(distinct(AbortKommuneAlleAldre[, 1] == i))  
  for (j in 1995:2018){ # check that correct columns are grabbed, use 2:ncol
    newvalue <- as.numeric(AbortNumDF[x,j])/as.numeric(PopulationMun[i,j])
